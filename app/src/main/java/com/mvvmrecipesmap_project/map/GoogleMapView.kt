...
private val helsinki = LatLng(61.9184, 24.87)
private val helsinki2 = LatLng(61.40, 24.77)
private val helsinki3 = LatLng(61.45, 24.77)
private val defaultCameraPosition = CameraPosition.fromLatLngZoom(helsinki, 11f)
...
fun GoogleMapView(
    ...
    val helsinkiState = rememberMarkerState(position = helsinki)
    val helsinki2State = rememberMarkerState(position = helsinki2)
    val helsinki3State = rememberMarkerState(position = helsinki3)
    var circleCenter by remember { mutableStateOf(helsinki) }
    if (helsinkiState.dragState == DragState.END) {
        circleCenter = helsinkiState.position
    }
    ...
    Column {
        ...
        Row {
            MapButton(
                ...
                onClick = {
                    ...
                    helsinkiState.position = helsinki
                    helsinkiState.hideInfoWindow()
                }
            )
            MapButton(
                text = "Toggle Map",
        ...
        DebugView(cameraPositionState, helsinkiState)
    }
}
